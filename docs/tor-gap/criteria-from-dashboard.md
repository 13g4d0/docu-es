# Evaluation criteria — snapshot from dashboard

This page mirrors the `CRITERIA` JavaScript array embedded in **`map/dashboard.html`** (devops repository). It is the **scoring rubric** used for gap analysis (sections **8.1–8.5**). If the dashboard script changes, update this snapshot in the same commit.

!!! note "Totals"
    Section maxima coded in the dashboard: **8.1 = 20**, **8.2 = 30**, **8.3 = 10**, **8.4 = 20**, **8.5 = 20**. Pre–technical-test scoring uses the experience + methodology + personnel buckets per dashboard logic (`threshold = 63` on a **90**-point pre-test scale in UI text).
...
```mermaid
flowchart TB
  TdR[TdR PDF official]
  EXT[Text extract _extracted/]
  DASH[Dashboard CRITERIA]
  ASBUILT[docu as-built pages]

  TdR --> EXT
  TdR --> DASH
  ASBUILT -->|implementation evidence| GAP[Gap workbook]
  DASH --> GAP
  EXT --> GAP
```
...
- [Gap workbook](gap-analysis-workbook.md)
