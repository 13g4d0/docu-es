...
`dev-stack.sh` resuelve `UI_ROOT` relativo al checkout del servicio RAG (`../open-webui` por defecto). Los operadores pueden sobrescribir con variables de entorno.
...
- **Negativo:** Clones en ubicaciones no estándar deben exportar `UI_ROOT` explícitamente.
